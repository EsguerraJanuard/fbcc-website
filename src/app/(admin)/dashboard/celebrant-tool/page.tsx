...
export default function CelebrantToolPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 animate-in fade-in duration-500">
      <div className="mb-8">
        <h1 className="text-3xl font-serif font-bold text-gray-900">Celebrant Canvas</h1>
      ...
      <div className="flex flex-col lg:flex-row gap-8 items-start">
        ...
        <div className="w-full lg:w-5/12 space-y-6 order-2 lg:order-1">
          <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100 space-y-8">
            ...
            {/* Uploads */}
            <div>
              <h3 className="text-xs font-bold uppercase tracking-widest text-fbcc-navy mb-4 border-b border-gray-100 pb-2">1. Upload Assets</h3>
              <div className="space-y-5">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">Overlay Frame (Transparent PNG)</label>
                  <input type="file" accept="image/png" onChange={(e) => handleImageUpload(e, "frame")} className="w-full text-sm text-gray-500 file:mr-4 file:py-2.5 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-bold file:bg-gray-100 file:text-gray-700 hover:file:bg-gray-200 cursor-pointer transition-colors" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">Person's Portrait (JPG/PNG)</label>
                  <input type="file" accept="image/*" onChange={(e) => handleImageUpload(e, "portrait")} className="w-full text-sm text-gray-500 file:mr-4 file:py-2.5 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-bold file:bg-gray-100 file:text-gray-700 hover:file:bg-gray-200 cursor-pointer transition-colors" />
                </div>
              </div>
            </div>
            ...
            <div className={!portraitImg ? "opacity-50 pointer-events-none transition-opacity" : "transition-opacity"}>
              <h3 className="text-xs font-bold uppercase tracking-widest text-fbcc-navy mb-4 border-b border-gray-100 pb-2 flex justify-between items-center">
                2. Adjust Portrait
                <button onClick={() => { setScale(baseScale); setOffsetX(0); setOffsetY(0); }} className="text-[10px] bg-gray-100 px-2 py-1 rounded text-gray-500 hover:bg-gray-200 transition-colors">Reset</button>
              </h3>
        ...
        <div className="w-full lg:w-7/12 order-1 lg:order-2 lg:sticky lg:top-8">
          <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100 flex flex-col items-center">
            <h3 className="text-xs font-bold uppercase tracking-widest text-fbcc-navy mb-6 self-start border-b border-gray-100 pb-2 w-full">Live Preview</h3>
            
            <div className="w-full aspect-square bg-gray-50 border-2 border-dashed border-gray-200 rounded-2xl overflow-hidden relative shadow-inner">
              {(!frameImg && !portraitImg) && (
                <div className="absolute inset-0 flex flex-col items-center justify-center text-gray-400 p-6 text-center">
                  ...
                  <p className="font-medium text-gray-500">Upload a frame and portrait to see preview</p>
                </div>
              )}
            ...
            <p className="text-xs font-bold text-gray-400 mt-6 uppercase tracking-widest bg-gray-50 px-4 py-2 rounded-lg">
